...
void DrawIRdE(){
  ...
  double xmin=-1,xmax=0.2,ymax=1e7;
  int nbin=100;
  TH1F *fIRdE_tous = new TH1F("fIRdE_tous","",nbin,xmin,xmax);
  TH1F *fIRdE_coul = new TH1F("fIRdE_coul","",nbin,xmin,xmax);
  TH1F *fIRdE_brem = new TH1F("fIRdE_brem","",nbin,xmin,xmax); 
  fIRdE_tous->SetMarkerColor(kRed);
  fIRdE_coul->SetMarkerColor(kGreen);
  fIRdE_brem->SetMarkerColor(kBlue);
  fIRdE_tous->SetMarkerStyle(5);
  fIRdE_coul->SetMarkerStyle(26);
  fIRdE_brem->SetMarkerStyle(4);
  fIRdE_tous->SetMarkerSize(1.5);
  fIRdE_coul->SetMarkerSize(1.5);
  fIRdE_brem->SetMarkerSize(1.5);
  fIRdE_tous->SetXTitle("dE/E0");
  fIRdE_tous->SetYTitle("Lost rate / Hz"); 
  fIRdE_tous->GetYaxis()->SetRangeUser(0,ymax);
  ...
  for(Int_t itous=0;itous<ntous;itous++){
    ...
    fIRdE_tous->Fill(dE_tous,dN_tous);
  }
  ...
  for(Int_t icoul=0;icoul<ncoul;icoul++){
    ...
    fIRdE_coul->Fill(dE_coul,dN_coul);
  }
  ...
  for(Int_t ibrem=0;ibrem<nbrem;ibrem++){
    ...
    fIRdE_brem->Fill(dE_brem,dN_brem);
  }
  ...
  fIRdE_tous->GetYaxis()->SetRangeUser(1,ymax);
  gPad->SetLogy();  
  ...
  leg->AddEntry(fIRdE_tous,"Touschek","p");
  leg->AddEntry(fIRdE_coul,"Coulomb","p");
  leg->AddEntry(fIRdE_brem,"Bremsstrahlung","p");
  
  fIRdE_tous->Draw("E");
  fIRdE_coul->Draw("Esame");
  fIRdE_brem->Draw("Esame");
  leg->Draw();

  c1->Print("Drawings/IRdE.eps");
}
